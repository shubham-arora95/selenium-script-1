...
public class TestSiteCopier {
	...
	String newPostUrl = "http://onlinebachat.in/wp-admin/post-new.php";
				...
				try {
					for (int i = 0; i < amazonDeals.size(); i++) {
						...
						if (!(postsList.contains(amazonDeals.get(i).findElements(By.tagName("a")).get(1).getText()))) {
							...
							// String ifsOriginalPrice =
							// amazonDeals.get(i).findElements(By.className("old-price")).get(1).getText();
							postIFSDeal(amazonLink, ifsTitle, "amazon");
						} else {
							break;
						}
					}
	...
	public void beforeTest() {
		...
		try {
			...
			loginIntoAmazon("onlinebachat.in@gmail.com", "Shubh@2640203");
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		logIntoBlog("onlinebachatadmin", "%xhj%RUM1&");
	}
	...
	public Map<String, String> getFieldsFromAmazon(String amazonURL, String ifsTitle) throws Exception {
		...
		if (count == 3 && (affilateLink == null || affilateLink.length() < 5)) {
			sendOnlyMessageOnTelegram(ifsTitle, driver.getCurrentUrl(), oldPrice, null);
			affilateLink = driver.getCurrentUrl();
			affilateLink = changeIFSAffilteToOurAffilate(affilateLink);
	...
	private String changeIFSAffilteToOurAffilate(String affilateLink) {
		boolean shortURL = false;
		if (affilateLink.contains("dealhind-21")) {
			affilateLink = affilateLink.replace("dealhind-21", "onlinebacha07-21");
			shortURL = true;
		}
		if (affilateLink.contains("indiafreestuff")) {
			affilateLink = affilateLink.replace("indiafreestuff", "onlinebachat");
			shortURL = true;
		}
	...
	public void postDealWrapper(String amazonURL, String ifsTitle) throws Exception {
		...
		if (null != returnMap.get("sendOnTelegram") && Boolean.TRUE.parseBoolean(returnMap.get("sendOnTelegram"))) {
			sendOnlyMessageOnTelegram(ifsTitle,
					"" != returnMap.get("affilateLink") ? returnMap.get("affilateLink") : driver.getCurrentUrl(),
					returnMap.get("oldPrice"), returnMap.get("couponDiscount"));
			postDeal(returnMap);
		}
	...
	public void postIFSDeal(String ifsAmazonLink, String ifsTitle, String dealType) throws Exception {
		js.executeScript("window.open('', '_blank');");

		ArrayList<String> tabs = new ArrayList(driver.getWindowHandles());
		String postTitle = null;
		String amazonLinkHref = null;
		postAmazonIFSDeal(ifsAmazonLink, ifsTitle, tabs, postTitle, amazonLinkHref);
	}

	private void postAmazonIFSDeal(String ifsAmazonLink, String ifsTitle, ArrayList<String> tabs, String postTitle,
	...
	public void postIFSFlipkartDeal(String ifsFlipkartLink, String ifsTitle) throws Exception {
		...
		try {
			...
			try {
				 ...
				 * ); try { Thread.sleep(2000); } catch (Exception e2) { // TODO: handle
				 * exception }
				 */
				try {
					/*
	...
	public void sendOnlyMessageOnTelegram(String linkText, String affilatedLink, String originalPrice,
			String dealPercentage) {
		driver.get("http://onlinebachat.in/wp-admin/admin.php?page=telegram_send");
		...
		affilatedLink = changeIFSAffilteToOurAffilate(affilatedLink);
